...
app.get('/store/:slug', (req, res) => {
  ...
  site.products.forEach(p => {
    const cat = p.categoryId && categoryMap[p.categoryId] ? categoryMap[p.categoryId] : { name: 'Products', image: '' };
    const key = cat.name;
    if (!grouped[key]) grouped[key] = { category: cat, products: [] };
    grouped[key].products.push(p);
  });

  const productsHtml = Object.keys(grouped).map(catName => {
    const { category, products } = grouped[catName];
    return `
    <div class="cat-section">
      ${Object.keys(grouped).length > 1 ? `
        <div style="display:flex;align-items:center;gap:12px;margin-bottom:16px;">
          ${category.image ? `<img src="${escapeHtml(category.image)}" style="width:44px;height:44px;object-fit:cover;border-radius:8px;"/>` : ''}
          <h2 class="cat-title" style="margin:0;">${escapeHtml(catName)}</h2>
        </div>` : ''}
