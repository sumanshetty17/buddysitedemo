...
function renderPageBlocks(content) {
  ...
  // Text-only: elegant standalone sections with a decorative divider between them.
  if (images.length === 0) {
    return paragraphs.map((b, i) => `
      ${i > 0 ? '<div style="text-align:center;color:#FF6A88;font-size:1.1rem;letter-spacing:.4em;margin:28px 0;">✦ ✦ ✦</div>' : ''}
      <p style="line-height:1.9;font-size:1.05rem;margin:0;text-align:center;max-width:640px;margin-left:auto;margin-right:auto;">${escapeHtml(b.text || '')}</p>
    `).join('');
  }

  // Both text and images: alternate image-left/text-right, then text-left/image-right, etc.
  ...
  for (let i = 0; i < pairCount; i++) {
    ...
    if (para && img) {
      html += `
      <div style="display:flex;gap:36px;align-items:center;margin-bottom:44px;flex-wrap:wrap;${reverse ? 'flex-direction:row-reverse;' : ''}">
        <img src="${escapeHtml(img.url || '')}" style="flex:1 1 320px;max-width:100%;border-radius:16px;box-shadow:0 4px 20px rgba(0,0,0,.08);"/>
        <div style="flex:1 1 320px;"><p style="line-height:1.8;font-size:1.02rem;margin:0;">${escapeHtml(para.text || '')}</p></div>
      </div>`;
    } else if (para) {
      html += `<p style="line-height:1.9;font-size:1.05rem;margin:0 0 28px;text-align:center;">${escapeHtml(para.text || '')}</p>`;
    } else if (img) {
      html += `<img src="${escapeHtml(img.url || '')}" style="width:100%;border-radius:16px;margin-bottom:28px;box-shadow:0 4px 20px rgba(0,0,0,.08);"/>`;
    }
  }
  return html;
}
