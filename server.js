...
// ---------- CATEGORY SECTIONS / GROUPS (Pro plan only) ----------
app.post('/api/sites/:id/category-groups', requireAuth, loadOwnedSite, (req, res) => {
  ...
  if (!Array.isArray(categoryIds) || categoryIds.length === 0) return res.status(400).json({ error: 'Please select at least one category for this section.' });
  const group = db.addCategoryGroup(req.site.id, { title, categoryIds });
  res.json({ group });
});
...
function renderHeroSlider(slides) {
  ...
  (function(){
    ...
    var timer;
    function resetTimer(){ clearInterval(timer); if (n > 1) timer = setInterval(function(){ go(idx + 1); }, 750); }
    resetTimer();
  })();
  </script>`;
}

// The full Pro-plan homepage: multi-slide hero, a "New Arrivals" carousel,
// admin-defined titled category sections (square tiles), a "Shop All"
// product grid with a sticky filter bar, and a brand-story + socials footer.
function renderProStorefront(site, plan, t) {
  ...
  const latestProducts = site.products.slice(-10).reverse();
  const newArrivalsHtml = latestProducts.length ? `
    <div class="section-wrap">
      <h2 class="section-title">New Arrivals</h2>
      <div class="carousel-wrap">
        <button class="carousel-arrow carousel-arrow-left" onclick="sbScrollCarousel('newArrivalsTrack',-1)" aria-label="Previous">‹</button>
        <div class="carousel-track" id="newArrivalsTrack">
          ${latestProducts.map(p => renderProductCard(p, t, site)).join('')}
        </div>
        <button class="carousel-arrow carousel-arrow-right" onclick="sbScrollCarousel('newArrivalsTrack',1)" aria-label="Next">›</button>
      </div>
    </div>` : '';

  // Custom sections show first, in the order the seller created them. Any
  // category that isn't in ANY section yet still needs to show up somewhere
  // -- so it appears in an auto "Categories" section appended after them.
  // (Previously this fallback only kicked in when there were zero sections
  // at all, which made categories vanish from the homepage as soon as the
  // seller made their first custom section.)
  const customGroups = (site.categoryGroups || []).filter(g => g.categoryIds && g.categoryIds.length);
  const groupedIds = new Set(customGroups.flatMap(g => g.categoryIds));
  const leftoverCategories = site.categories.filter(c => !groupedIds.has(c.id));
  const groups = customGroups.concat(
    leftoverCategories.length ? [{ id: 'default', title: customGroups.length ? 'Categories' : 'Categories', categoryIds: leftoverCategories.map(c => c.id) }] : []
  );

  const categoryGroupsHtml = groups.map(g => {
    const cats = g.categoryIds.map(id => categoryMap[id]).filter(Boolean);
