...
function renderProductCard(p, t, extraAttrs) {
  return `
    ...
    <div class="section-wrap">
      ...
      <div class="carousel-wrap">
        ...
        <div class="carousel-track" id="newArrivalsTrack">
          ${latestProducts.map(p => renderProductCard(p, t)).join('')}
        </div>
        <button class="carousel-arrow carousel-arrow-right" onclick="sbScrollCarousel('newArrivalsTrack',1)" aria-label="Next">›</button>
      </div>
    </div>` : '';

  // If the seller hasn't grouped their categories into named sections yet,
  // fall back to one default "Categories" section containing all of them.
  const groups = (site.categoryGroups && site.categoryGroups.length)
    ? site.categoryGroups
    : (site.categories.length ? [{ id: 'default', title: 'Categories', categoryIds: site.categories.map(c => c.id) }] : []);
  ...
  const shopAllHtml = allProducts.length ? `
    <div class="section-wrap">
      ...
      <div class="product-grid" id="shopAllGrid">
        ${allProducts.map(p => renderProductCard(p, t, ` data-category="${p.categoryId || ''}"`)).join('')}
      </div>
    </div>` : '';
...
<style>
  *{box-sizing:border-box;}
  ...
  .price{font-size:1.2rem;font-weight:700;color:${t.accent};margin:4px 0 12px;}
...
app.get('/store/:slug', (req, res) => {
  ...
  const uncategorizedHtml = uncategorized.length ? `
    <div class="cat-section">
      <div class="product-grid">
        ${uncategorized.map(p => renderProductCard(p, t)).join('')}
      </div>
    </div>` : '';
