...
const { PLANS, INTRO_MONTHS, INTRO_AMOUNT_PAISE, PLATFORM_FEE_PERCENT } = require('./plans');
...
// ---------- BILLING (Razorpay) ----------
app.get('/api/billing/plans', (req, res) => res.json({ plans: Object.values(PLANS), razorpayEnabled, introMonths: INTRO_MONTHS, introAmountPaise: INTRO_AMOUNT_PAISE }));

// ============================================================
// ⚠️ DEMO-ONLY ROUTE — instantly activates a plan, no payment.
// This exists ONLY so you can preview the post-payment experience
// yourself. NEVER deploy this build anywhere a real customer could
// reach it — anyone could get a free subscription.
// ============================================================
...
app.post('/api/billing/order', requireAuth, async (req, res) => {
  ...
  const paidCycles = db.countPaidCyclesForUser(req.userId);
  const amount = paidCycles < INTRO_MONTHS ? INTRO_AMOUNT_PAISE : planConfig.amount_paise;
  ...
  try {
    ...
    res.json({ order, keyId: process.env.RAZORPAY_KEY_ID, isIntroPrice: amount === INTRO_AMOUNT_PAISE });
  } catch (err) {
    console.error(err);
...
window.sbPlaceOrder = function(){
  ...
  var payload = {
    ...
    phone: phone,
    address: address,
    paymentMethod: document.getElementById('sb-payment').value,
    items: sbGetCart()
  };
