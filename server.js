...
// Money helper -- keeps commission math exact instead of drifting from
// unrounded floating-point arithmetic.
...
app.put('/api/sites/:id/orders/:orderId', requireAuth, loadOwnedSite, (req, res) => {
  ...
  // refundAmount only applies to 'refunded' -- omit it (or send the full
  // order total) for a full refund; a smaller number does a partial
  // refund and reverses BuddySite's commission proportionally.
  const order = db.updateOrderStatus(req.site.id, req.params.orderId, status, refundAmount);
  if (!order) return res.status(404).json({ error: 'Order not found.' });
  res.json({ order });
});

// ---------- FINANCE / PAYOUTS (commission transparency for sellers) ----------
app.get('/api/sites/:id/finance', requireAuth, loadOwnedSite, (req, res) => {
  res.json({
    summary: db.getFinanceSummaryForSite(req.site.id),
    ledger: db.getCommissionsForSite(req.site.id)
  });
});
...
app.post('/api/public/sites/:slug/order', (req, res) => {
  ...
  const total = round2(items.reduce((sum, i) => sum + (Number(i.price) || 0) * (Number(i.qty) || 1), 0));

  // BuddySite transaction commission -- calculated server-side, on the
  // merchandise subtotal, using the SELLER'S CURRENT plan at the moment
  // the order is placed. That rate is then frozen onto the order/ledger
  // permanently: if the seller changes plans later, this order's
  // commission does not change retroactively.
  const seller = db.getUserById(site.user_id);
  const sellerPlanKey = (seller && PLANS[seller.plan]) ? seller.plan : 'free';
  const commissionRate = PLANS[sellerPlanKey].commission_percent;
  const commissionAmount = round2(total * commissionRate / 100);
  const commission = {
    plan_at_transaction: sellerPlanKey,
    rate: commissionRate,
    base_amount: total,
    amount: commissionAmount,
    currency: 'INR',
    seller_payout_amount: round2(total - commissionAmount),
    refund_adjustment_amount: 0,
    status: 'pending'
  };

  const order = db.addOrder(site.id, { customerName, email, phone, address, paymentMethod, items, total, commission });
  db.createCommissionRecord({
    order_id: order.id, store_id: site.id, seller_id: site.user_id,
    plan_at_transaction: sellerPlanKey, commission_rate_at_transaction: commissionRate,
    commission_base_amount: total, commission_amount: commissionAmount,
    currency: 'INR', seller_payout_amount: commission.seller_payout_amount,
    status: 'pending', refund_adjustment_amount: 0, event: 'order_created'
  });
  res.json({ ok: true, order });
});

// ---------- BILLING (Razorpay) ----------
app.get('/api/billing/plans', (req, res) => res.json({ plans: Object.values(PLANS), razorpayEnabled }));

// The Free plan needs no payment at all -- just activate it.
app.post('/api/billing/activate-free', requireAuth, (req, res) => {
  const user = db.updateUserPlan(req.userId, 'free', null);
  db.createPayment({ user_id: req.userId, plan: 'free', amount_paise: 0, razorpay_order_id: 'free_' + Date.now(), status: 'paid' });
  res.json({ ok: true, user: publicUser(user) });
});

// ============================================================
// ⚠️ DEMO-ONLY ROUTE — instantly activates a PAID plan without
// payment, and without auto-pay setup. This exists ONLY so you
// can preview the post-payment experience yourself. NEVER deploy
// this build anywhere a real customer could reach it.
// ============================================================
...
app.post('/api/billing/order', requireAuth, async (req, res) => {
  ...
  if (plan === 'free') return res.status(400).json({ error: 'The Free plan does not require payment -- use Activate instead.' });

  // Flat monthly price -- the same amount is charged every cycle now,
  // whether paid manually or via auto-pay, so there is no separate
  // "first month" price to track anymore.
  const amount = planConfig.amount_paise;
...
// Ensures a Razorpay "Plan" object exists for this tier at its flat monthly
// price, creating it once and caching the id so we never create duplicates.
...
app.post('/api/billing/enable-autopay', requireAuth, async (req, res) => {
  ...
  if (user.plan === 'free') return res.status(400).json({ error: 'The Free plan is ₹0 -- auto-pay is not needed.' });
...
function renderTopbar(site, current, t) {
  ...
  const videoPosition = site.logoVideoPosition === 'right' ? 'right' : 'left';
  ...
  return `<div class="sb-topbar" style="background:${t.topbarBg};border-bottom:1px solid ${t.border};position:sticky;top:0;z-index:5;">
    <div style="width:100%;padding:${barPadding};display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:16px;box-sizing:border-box;">
      ...
      <div style="text-align:center;">${namePosition === 'center' ? storeNameHtml : ''}</div>
...
(function(){
  ...
  function render(){
    ...
    document.getElementById('sb-cart-page-total').textContent = money(subtotal);
