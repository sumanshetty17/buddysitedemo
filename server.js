...
app.get('/store/:slug', (req, res) => {
  ...
  const categoriesWithProducts = site.categories.filter(c => site.products.some(p => p.categoryId === c.id));
  ...
  const categoryTilesHtml = categoriesWithProducts.length ? `
    <div class="cat-section">
      ...
      <div class="category-grid">
        ${categoriesWithProducts.map(c => `
          <a class="category-tile" href="/store/${site.slug}/category/${c.id}">
            ${(plan.categoryImages && c.image) ? `<img src="${escapeHtml(c.image)}" alt="${escapeHtml(c.name)}"/>` : '<div class="category-tile-noimg">🛍️</div>'}
...
<style>
  *{box-sizing:border-box;}
  ...
  .category-tile{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:10px;background:#F3F3F1;border:1.5px solid #D9D9D6;border-radius:${t.radius};padding:20px 12px;text-decoration:none;color:#1A1A1A;font-weight:600;text-align:center;transition:transform .12s ease;}
  .category-tile:hover{transform:translateY(-2px);border-color:#B5B5B0;}
  .category-tile img{width:64px;height:64px;object-fit:cover;border-radius:8px;}
  .category-tile-noimg{width:64px;height:64px;display:flex;align-items:center;justify-content:center;font-size:1.8rem;background:#E8E8E5;border-radius:8px;}
  .product-card{background:${t.cardBg};border:1px solid ${t.border};border-radius:${t.radius};overflow:hidden;}
  .product-card img{width:100%;height:180px;object-fit:cover;}
  .no-img{width:100%;height:180px;background:${t.border};display:flex;align-items:center;justify-content:center;color:${t.muted};}
  .product-body{padding:16px;}
