...
app.put('/api/sites/:id/colors', requireAuth, loadOwnedSite, (req, res) => {
  const result = db.updateSiteColors(req.site.id, req.body || {});
...
app.post('/api/public/sites/:slug/order', (req, res) => {
  ...
  const order = db.addOrder(site.id, { customerName, email, phone, address, paymentMethod, items: priceable, total, couponCode: appliedCoupon ? appliedCoupon.code : '', discount, subtotal, whatsapp_opt_in, customer_id: customerId });
  ...
  if (customerId) db.setCustomerCart(site.id, customerId, []); // order placed -- clear the account's saved cart
...
app.post('/api/public/sites/:slug/customer/signup', (req, res) => {
  const site = db.getSiteBySlug(req.params.slug);
  if (!site || !site.published) return res.status(404).json({ error: 'Store not found.' });
  ...
  if ((email && db.getCustomerByIdentifier(site.id, email)) || db.getCustomerByIdentifier(site.id, phone)) {
    return res.status(400).json({ error: 'An account with that email or phone already exists on this store.' });
  }
  const hash = bcrypt.hashSync(password, 10);
  const customer = db.createCustomer(site.id, { name, email, phone, password_hash: hash });
  if (address) db.addCustomerAddress(site.id, customer.id, { label: 'Home', name, phone, address, isDefault: true });
...
app.post('/api/public/sites/:slug/customer/login', (req, res) => {
  const site = db.getSiteBySlug(req.params.slug);
  if (!site || !site.published) return res.status(404).json({ error: 'Store not found.' });
  const { identifier, password } = req.body || {};
  const customer = db.getCustomerByIdentifier(site.id, identifier || '');
...
app.post('/api/public/sites/:slug/customer/otp/verify', (req, res) => {
  const site = db.getSiteBySlug(req.params.slug);
  if (!site || !site.published) return res.status(404).json({ error: 'Store not found.' });
  ...
  db.updateCustomerPassword(site.id, customer.id, bcrypt.hashSync(newPassword, 10));
  res.json({ token: makeCustomerToken(customer.id, site.id), customer: publicCustomer(db.getCustomerById(site.id, customer.id)) });
});

app.get('/api/public/sites/:slug/customer/me', requireCustomerAuth, (req, res) => {
  const customer = db.getCustomerById(req.site.id, req.customerId);
  if (!customer) return res.status(404).json({ error: 'Account not found.' });
  res.json({ customer: publicCustomer(customer) });
});

app.put('/api/public/sites/:slug/customer/me', requireCustomerAuth, (req, res) => {
  const { name, email, phone } = req.body || {};
  const customer = db.updateCustomerProfile(req.site.id, req.customerId, { name, email, phone });
  if (!customer) return res.status(404).json({ error: 'Account not found.' });
  res.json({ customer: publicCustomer(customer) });
});

// ----- Saved addresses -----
app.post('/api/public/sites/:slug/customer/addresses', requireCustomerAuth, (req, res) => {
  const { label, name, phone, address, isDefault } = req.body || {};
  if (!address) return res.status(400).json({ error: 'Address is required.' });
  const entry = db.addCustomerAddress(req.site.id, req.customerId, { label, name, phone, address, isDefault });
  if (!entry) return res.status(404).json({ error: 'Account not found.' });
  res.json({ address: entry });
});
app.put('/api/public/sites/:slug/customer/addresses/:addressId', requireCustomerAuth, (req, res) => {
  const { label, name, phone, address, isDefault } = req.body || {};
  const entry = db.updateCustomerAddress(req.site.id, req.customerId, req.params.addressId, { label, name, phone, address, isDefault });
  if (!entry) return res.status(404).json({ error: 'Address not found.' });
  res.json({ address: entry });
});
app.delete('/api/public/sites/:slug/customer/addresses/:addressId', requireCustomerAuth, (req, res) => {
  const deleted = db.deleteCustomerAddress(req.site.id, req.customerId, req.params.addressId);
  if (!deleted) return res.status(404).json({ error: 'Address not found.' });
  res.json({ ok: true });
});

// ----- Cart, synced to the account while logged in -----
app.get('/api/public/sites/:slug/customer/cart', requireCustomerAuth, (req, res) => {
  const cart = db.getCustomerCart(req.site.id, req.customerId);
  if (cart === null) return res.status(404).json({ error: 'Account not found.' });
  res.json({ cart });
});
app.put('/api/public/sites/:slug/customer/cart', requireCustomerAuth, (req, res) => {
  const cart = db.setCustomerCart(req.site.id, req.customerId, (req.body || {}).cart || []);
  if (cart === null) return res.status(404).json({ error: 'Account not found.' });
  res.json({ cart });
});

// ----- Order history -----
app.get('/api/public/sites/:slug/customer/orders', requireCustomerAuth, (req, res) => {
  res.json({ orders: db.getOrdersForCustomer(req.site.id, req.customerId) });
});
...
app.get('/store/:slug/login', (req, res) => {
  const site = db.getSiteBySlug(req.params.slug);
  if (!site || !site.published) return res.status(404).send('<h1>Store not found or not published yet.</h1>');
  res.send(renderLoginPage(site, getTheme(site)));
});

app.get('/store/:slug/account', (req, res) => {
  const site = db.getSiteBySlug(req.params.slug);
  if (!site || !site.published) return res.status(404).send('<h1>Store not found or not published yet.</h1>');
