...
function renderPageBlocks(content) {
  return (content || []).map(b => {
    if (b.type === 'paragraph') return `<p style="line-height:1.7;margin-bottom:16px;">${escapeHtml(b.text || '')}</p>`;
    if (b.type === 'image') return `<img src="${escapeHtml(b.url || '')}" style="max-width:100%;border-radius:12px;margin-bottom:16px;"/>`;
    return '';
  }).join('');
}
...
<div class="container">${renderPageBlocks(page.content) || '<p style="color:#a8a29e;">Nothing here yet.</p>'}</div>
