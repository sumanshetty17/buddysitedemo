...
function getTheme(themeKey) {
  const themes = {
    simple: {
      ...
      fontsUrl: 'https://fonts.googleapis.com/css2?family=Poppins:wght@600;700&family=Inter:wght@400;500;600&display=swap',
    ...
    bold: {
      ...
      headingFont: "'Poppins',sans-serif", bodyFont: "'Inter',system-ui,sans-serif",
      fontsUrl: 'https://fonts.googleapis.com/css2?family=Poppins:wght@700;800&family=Inter:wght@400;500;600&display=swap',
    ...
    aesthetic: {
      ...
      fontsUrl: 'https://fonts.googleapis.com/css2?family=Playfair+Display:wght@600;700&family=Inter:wght@400;500;600&display=swap',
...
function renderTopbar(site, current, t) {
  ...
  return `<div style="background:${t.topbarBg};border-bottom:1px solid ${t.border};position:sticky;top:0;z-index:5;">
...
function renderHero(bgUrl, bgType, t, captionHtml) {
  const bg = bgUrl
    ? (bgType === 'video'
    ...
    : '';
  return `<div style="position:relative;min-height:${bgUrl ? '320px' : '160px'};display:flex;align-items:flex-end;padding:28px 24px;overflow:hidden;${bgUrl ? '' : 'background:' + t.heroFallback + ';'}">
    ${bg}
    ${captionHtml ? `<div style="position:relative;z-index:1;background:${t.cardBg};color:${t.ink};padding:10px 20px;border-radius:${t.btnRadius === '100px' ? '100px' : '6px'};font-size:.95rem;max-width:560px;box-shadow:0 2px 12px rgba(0,0,0,.15);">${captionHtml}</div>` : ''}
  </div>`;
}
...
app.get('/store/:slug', (req, res) => {
  ...
  const hero = renderHero(site.background_url, site.background_type, t, site.description ? escapeHtml(site.description) : '');
