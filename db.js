...
function empty() {
  return {
    ...
    nextHeroSlideId: 1, nextCategoryGroupId: 1, nextSlidingSectionId: 1, nextCouponId: 1
  };
}
...
// ---------- ORDERS ----------
function addOrder(siteId, { customerName, email, phone, address, paymentMethod, items, total, couponCode, discount }) {
  const data = load();
  const site = data.sites.find(s => s.id === Number(siteId));
  if (!site) return null;
