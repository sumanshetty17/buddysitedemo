...
function empty() {
  return {
    ...
    nextHeroSlideId: 1, nextCategoryGroupId: 1, nextSlidingSectionId: 1
  };
}
...
function withPageDefaults(site) {
  ...
  if (Array.isArray(site.products)) {
    site.products.forEach(p => { if (!Array.isArray(p.images)) p.images = p.image ? [p.image] : []; });
  }
...
// ---------- PRODUCTS ----------
function addProduct(siteId, { name, price, image, images, size, color, description, categoryId }) {
  const data = load();
  const site = data.sites.find(s => s.id === Number(siteId));
  if (!site) return null;
  const imgList = (Array.isArray(images) && images.length) ? images.filter(Boolean) : (image ? [image] : []);
  const product = { id: data.nextProductId++, name, price: Number(price) || 0, image: imgList[0] || '', images: imgList, size: size || '', color: color || '', description: description || '', categoryId: categoryId || null };
...
function updateProduct(siteId, productId, fields) {
  ...
  ['name', 'price', 'size', 'color', 'description', 'categoryId'].forEach(f => {
