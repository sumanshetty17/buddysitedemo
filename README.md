...
- **New pricing (per-plan discounted first month, then regular price via auto-pay)**:
  - **Basic**: ₹20 first month → ₹99/mo from month 2 (marketed as ₹330, 70% off)
  - **Grow**: ₹59 first month → ₹299/mo from month 2 (marketed as ₹997, 70% off)
  - **Pro**: ₹99 first month → ₹599/mo from month 2 (marketed as ₹1,997, 70% off)
- **Auto-pay is now part of subscribing, not optional**: after the discounted first-month
  payment succeeds, the checkout immediately continues into authorizing auto-pay (at the
  *regular* price — see the Auto-pay section below for why). If someone closes that second
  step, their first month still works, but they'll see a warning banner until they finish
  auto-pay setup or renew manually.
...
- **Background images** on the store, About page, and Contact page — every plan.
  **Background videos** are Grow and Pro only (Basic is blocked with a clear message).
- **Store admin panel** with a sidebar: Store Settings, About Us Page, Contact Us Page,
  Add Product, Manage Products, Categories (Grow+), Payment Method, Orders
...
- **Orders inbox** — every order a customer places shows up for the seller with full
  contact + delivery details, plus payment status
